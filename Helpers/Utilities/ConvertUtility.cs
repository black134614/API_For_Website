...
"d-M-yyyy", "dd-MM-yyyy", "d-M-yyyy HH:mm:ss", "d-M-yyyy HH:mm", "dd-MM-yyyy HH:mm", "HH:mm:ss", "HH:mm"};//HH phủ cả từ 1-24h còn hh chỉ phủ từ 1-12h
        ...
        public static string ToVNDCurrency(this object value, bool isUnit = true)
        {
            if (value == null) return "0 VNĐ";
            else return isUnit ? string.Format("{0:0,00} VND", value) : string.Format("{0:0,00}", value);
        }

        public static string ConvertStringToUnsignLetter(string input)
        {
            // Xóa dấu trong chuỗi
            string result = new string(input
                .Normalize(System.Text.NormalizationForm.FormD) // Chuyển chuỗi về dạng Unicode
                .Where(c => CharUnicodeInfo.GetUnicodeCategory(c) != UnicodeCategory.NonSpacingMark) // Lọc ra các ký tự không phải dấu
                .ToArray());
